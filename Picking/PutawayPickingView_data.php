...
} else if ($type > 30 && $type <= 40) //delete
{
	...
	if ($type == 31) {
		...
		try {
			...
			$sql = "UPDATE tbl_inventory_line t1,
			(SELECT fg_tag_no, from_location_id
			FROM tbl_transaction_line 
			WHERE transaction_id = uuid_to_bin('$transaction_id',true) 
			AND status != 'Cancel' AND fg_tag_no IS NOT NULL) t2
			SET t1.location_id = t2.from_location_id,
				t1.updated_at = NOW(), 
				t1.updated_user_id = $cBy
			WHERE t1.fg_tag_no = t2.fg_tag_no;";
			...
			FROM tbl_transaction_line 
			WHERE transaction_id = uuid_to_bin('$transaction_id',true) AND status != 'Cancel' GROUP BY case_tag_no) t2
			SET t1.location_id = t2.from_location_id,
				t1.updated_at = NOW(), 
				t1.updated_user_id = $cBy
			...
			$sql = "SELECT t4.part_tag_no, bin_to_uuid(t1.from_location_id,true) location_id
			FROM tbl_transaction_line t1
			inner join tbl_inventory t2 ON t1.case_tag_no = t2.case_tag_no
			inner join tbl_inventory_detail t4 ON t2.inventory_id = t4.inventory_id
			WHERE transaction_id = uuid_to_bin('$transaction_id',true) AND status != 'Cancel'
			AND fg_tag_no IS NULL
			order by part_tag_no;";
			$re1 = sqlError($mysqli, __LINE__, $sql, 1);
			if ($re1->num_rows == 0) {
				// throw new Exception('ไม่พบข้อมูล ' . __LINE__);
...
} else if ($type > 50 && $type <= 60) //export
{
	...
	if ($type == 51) {
		...
		$filename = "Data PutawayPicking " . $Date . "_" . $randomString . ".xlsx";
...
function select_group($mysqli, $data)
{
	...
	$sql = "SELECT 
		...
		t2.remark,
		concat(t4.location_code , ' | ', t4.location_area) as from_location,
		concat(t5.location_code , ' | ', t5.location_area) as to_location
	FROM
		tbl_transaction t1
	...
	WHERE t1.transaction_type = 'Move'
		...
		AND substring(document_no, 1, 2) = 'PM'
		$sqlWhere 
