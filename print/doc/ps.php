...
$q1  = "SELECT
		...
		Remark,
		Status_Picking,
		tph.Creation_DateTime,
		tph.Created_By_ID,
		tcm.Customer_Name  as Supplier
		FROM
		tbl_picking_header tph
		LEFT JOIN tbl_picking_pre tpp ON
		tph.Picking_Header_ID = tpp.Picking_Header_ID
		INNER JOIN tbl_part_master tpm ON
		tpp.Part_ID = tpm.Part_ID
		INNER JOIN tbl_customer_master tcm ON
		tcm.Customer_ID = tpm.Customer_ID WHERE tph.PS_Number= '$doc' GROUP BY tph.PS_Number ;";
...
tpm.Package_ID = tpm2.Package_ID WHERE tph.PS_Number= '$doc';";
...
class PDF extends PDF_Code128
{
	...
	public function setHeaderData($v)
	  ...
	  Fax : +66 38 058 007
	  ', 'valign:M;align:L');
      $header->easyCell($v[0]['PS_Number'], 'valign:B;align:C');
      $header->printRow();
      $header->endTable(2);
        ...
        $header->easyCell(utf8Th($v[0]['PS_Number']), 'valign:T;align:L;');
		...
		$this->instance->Code128(145,10,$v[0]['PS_Number'],55,7);
  	}
  	function Footer()
  	{
  		$this->SetXY(-20,0);
