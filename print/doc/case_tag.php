...
$dataset = array();
$q1  = "SELECT 
	part_no, part_name, t1.pallet_no, t1.case_tag_no, t1.qty, t1.net_per_pallet, t1.certificate_no,
    date_format(t5.document_date,'%d-%m-%Y') document_date
FROM
	tbl_inventory t1
	inner join tbl_part_master t2 ON t1.part_id = t2.part_id
    inner join tbl_transaction_line t4 ON t1.transaction_line_id = t4.transaction_line_id
    inner join tbl_transaction t5 ON t4.transaction_id = t5.transaction_id
WHERE
	t1.case_tag_no = '$doc';";

/* $q1  .= "SELECT 
	part_no, part_name, part_case_tag_no, 
	qty, net_per_pcs
FROM
...
$pdf->AddPage();
$docno = $headerData[0]['case_tag_no'];
$pdf->SetTitle($docno);
...
while ($i <  $data) {
	...
	$detail->easyCell(utf8Th($headerData[$i]["case_tag_no"]), 'align:C;border:TB;');
	...
	$pdf->Code128($x + 82, $y + 26.5, $headerData[$i]['case_tag_no'], 85, 12);
	$detail->printRow();
	...
	$detail->easyCell(utf8Th($headerData[$i]["pallet_no"]), 'align:C;');
	$detail->printRow();
	...
	$detail->easyCell(utf8Th($headerData[$i]["document_date"]), 'align:C;');
	$detail->printRow();
	...
	$detail->easyCell(utf8Th('Barcode'), 'font-size:10;valign:M; align:C; rotate:90;');
	$detail->easyCell(utf8Th(''), 'align:C;');
	$pdf->Code128($x + 82, $y + 171.5, $headerData[$i]['case_tag_no'], 85, 12);
	$detail->printRow();
