	...
	if ($type == 11) {
		...
		try {
			...
			$sql = "INSERT INTO tbl_transaction_line 
				( pallet_no, case_tag_no, part_id, qty, gross_kg, net_per_pallet, measurement_cbm, certificate_no, remark, package_type,
				transaction_id, to_location_id, invoice_no,
				created_at, updated_at, created_user_id, updated_user_id
				)
				VALUES 
				( '$pallet_no', '$case_tag_no', uuid_to_bin('$part_id',true), $qty, '$gross_kg', '$net_per_pallet', '$measurement_cbm', '$certificate_no', '$remark', '$package_type',
			...
			foreach ($text_plan as $row) {
				if (strpos(strtolower($row), 'sub-total') !== false) {
					...
					if ($list[0] == '') {
						...
						$certificate_no = str_replace(' ', '', $list[5]);
						$pallet_no = $list[6];
						$qty = trim($list[7]);
					} else {
						$gross_kg = $list[1];
						$net_per_pallet = $list[2];
						$measurement_cbm = $list[3];
						$certificate_no = str_replace(' ', '', $list[4]);
						$pallet_no = $list[5];
						$qty = trim($list[6]);
					}
		...
		try {
			...
			$sql = "SELECT pallet_no, bin_to_uuid(transaction_line_id,true) transaction_line_id FROM tbl_transaction_line WHERE transaction_id = uuid_to_bin('$transaction_id',true) 
			AND pallet_no = '$pallet_no';";
			$re1 = sqlError($mysqli, __LINE__, $sql, 1);
			if ($re1->num_rows == 0) {
				throw new Exception('ไม่พบ Pallet No. นี้' . __LINE__);
			}
			while ($row = $re1->fetch_array(MYSQLI_ASSOC)) {
				$transaction_line_id = $row['transaction_line_id'];
			}

			$sql = "SELECT pallet_no, bin_to_uuid(transaction_line_id,true) transaction_line_id FROM tbl_transaction_line 
			WHERE transaction_id = uuid_to_bin('$transaction_id',true) 
			AND pallet_no = '$pallet_no' AND receive_check = 'Yes';";
			$re1 = sqlError($mysqli, __LINE__, $sql, 1);
			if ($re1->num_rows > 0) {
				throw new Exception('Pallet No. นี้ สแกนไปแล้ว' . __LINE__);
			}

			$sql = "UPDATE tbl_transaction_line
			SET receive_check = 'Yes'
			WHERE transaction_id = uuid_to_bin('$transaction_id',true)
			AND transaction_line_id = uuid_to_bin('$transaction_line_id',true);";
			sqlError($mysqli, __LINE__, $sql, 1, 1);
			if ($mysqli->affected_rows == 0) {
...
} else if ($type > 40 && $type <= 50) //save
{
	...
	if ($type == 41) {
		...
		try {
			...
			$sql = "SELECT pallet_no, bin_to_uuid(transaction_line_id,true) transaction_line_id FROM tbl_transaction_line 
			WHERE transaction_id = uuid_to_bin('$transaction_id',true) AND receive_check = 'No';";
			$re1 = sqlError($mysqli, __LINE__, $sql, 1);
			if ($re1->num_rows > 0) {
				throw new Exception('ยังสแกนเช็ค Pallet ไม่ครบ');
			}
