...
} else if ($type > 30 && $type <= 40) //delete
{
	...
	if ($type == 31) {
		...
		try {
			...
			if ($re1->num_rows == 0) {
				throw new Exception('ไม่สามารถยกเลิกได้<br>ออเดอร์อยู่ในขั้นตอนดำเนินการ');
			}
...
function select_group($mysqli, $data)
{
	...
	try {
		...
		SELECT order_header_id, part_id, SUM(net_per_pcs) sum_net, SUM(qty) sum_qty FROM tbl_order
		WHERE status = 'Complete'
		GROUP BY order_header_id, part_id )
		SELECT
			dos_no, document_date,
			order_no, order_date, delivery_date, order_status, delivery_status,
			pallet_no, case_tag_no, t2.part_tag_no, part_no, part_name,
			t2.qty, t2.net_per_pcs, supplier_code,
			if(ROW_NUMBER() OVER(partition by order_no, part_no order by order_date DESC, delivery_date DESC, order_no DESC, part_no ASC, t2.part_tag_no ASC) = 1, sum_net, '') sum_net,
			if(ROW_NUMBER() OVER(partition by order_no, part_no order by order_date DESC, delivery_date DESC, order_no DESC, part_no ASC, t2.part_tag_no ASC) = 1, sum_qty, '') sum_qty,
			ROW_NUMBER() OVER(partition by order_no order by order_date DESC, delivery_date DESC, order_no DESC, part_no ASC, t2.part_tag_no ASC) row_num,
		...
		FROM
			tbl_order_header t1
				...
				INNER JOIN sum_net_by_part t5 ON t2.order_header_id = t5.order_header_id AND t2.part_id = t5.part_id
		...
		$sqlWhere
		order by order_date DESC, delivery_date DESC, order_no DESC, part_no ASC, t2.order_id ASC;";
		//exit($sql);
		$re1 = sqlError($mysqli, __LINE__, $sql, 1);
		return $re1;
	} catch (Exception $e) {
